...
import argparse
import os
from sklearn.feature_extraction import image as iimmgg

def extract_feature(image, type):
	if type == "stat1" : 
...
ap.add_argument("-t", "--testimg", type=str, default="",
	help="image file name to be tested")
ap.add_argument("-f", "--featuretype", type=str, default="stat1",
	help="feature type")
...
for imagePath in imagePaths:
	...
	features = extract_feature(image, args["featuretype"])
...
if  args["testimg"] != "" :
	print("[INFO] testimage : ", args["testimg"])
	tstimg = Image.open(args["testimg"])
	tstftr = []
	tstftr.append(extract_color_stats(tstimg))
	tstpred = model.predict(tstftr)
	print("[INFO] prediction : ", le.classes_[tstpred])
