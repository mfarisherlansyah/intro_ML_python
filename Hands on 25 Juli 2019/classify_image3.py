...
ap.add_argument("-f", "--feature_type", type = str, default = "stat1", help = "path to directory containing the '3scenes' dataset")
ap.add_argument("-ti", "--test_image", type = str, default = "", help = "image file you would like to test")
args = vars(ap.parse_args())

print("[INFO] Feature type: ", args["feature_type"])
...
def extract_color_stats2(image):
	# Split the input image into its respective RGB colour channels and then create a feature vector with six values; the mean and std for each of the three channels
	(R, G, B) = image.split()
	features = [np.mean(R), np.mean(G), np.mean(B), np.std(R), np.std(G), np.std(B), np.min(R), np.min(G), np.min(B), np.max(R), np.max(G), np.max(B)] # Ini jadi fitur-fiturnya
	# Return the set of features
	return features
...
def extract_color_stats3(image):
	# Split the input image into its respective RGB colour channels and then create a feature vector with six values; the mean and std for each of the three channels
	(R, G, B) = image.split()
	features = [np.mean(R), np.mean(G), np.mean(B), np.std(R), np.std(G), np.std(B), np.median(R), np.median(G), np.median(B)] # Ini jadi fitur-fiturnya
	# Return the set of features
	return features

def extract_feature(image, featuretype):
	if featuretype == "stat1":
		return extract_color_stats(image) # Extract fiturnya
	elif featuretype == "stat2":
		return extract_color_stats2(image)
	elif featuretype == "stat3":
		return extract_color_stats3(image)
	else:
		return extract_color_stats(image)
...
for imagePath in imagePaths:
	...
	features = extract_feature(image, args["feature_type"])
...
if args["test_image"] != "":
	print("[INFO] mikir dulu ya...")
	print("test image: ", args["test_image"])

	testimg = Image.open(imagePath)
	testimgfeature = []
	testimgfeature.append(extract_feature(testimg, args["feature_type"])) # Extract fiturnya
	testimgpredict = model.predict(testimgfeature)

	print("prediction: ", le.classes_[testimgpredict])